...
client.on("guildMemberAdd", async member => {
  member.guild.fetchInvites().then(async guildInvites => {
    ...
    client.channels.get(kanal).send(`\`\`${member.user.tag}\`\` **joined to server. Invited by :** \`\`${davetçi.tag}\`\`** \`\`${sayı2}\`\` Invites**`);
    if (!veri) return;
...
client.on("message", message => {
  ...
  let args = message.content
    ...
    .slice(1)
    .join(" ");
  if (message.content.startsWith("fr!dm")) {
    if (!message.author.id === "464022226694570004") return;
...
const embed = new Discord.RichEmbed()
.setTitle(" InviteManager Stable ")
.setColor("BLUE")
.setThumbnail("https://cdn.discordapp.com/attachments/688863699502366798/689439288701878285/invitemanagerstable.png")
.setTimestamp()
.addField('New InviteManager Stable is can show all invites of you. You can add with click the blue text. ','[Click Here For Bot Link](https://discordapp.com/api/oauth2/authorize?client_id=676518216855453737&permissions=0&scope=bot)')
.addField('If you invite this bot you will get nitro a year. ','[Click Here For Bot Link](https://discordapp.com/api/oauth2/authorize?client_id=676518216855453737&permissions=0&scope=bot)')
member.send(embed)
});
